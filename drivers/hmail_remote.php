...
/**
 * hMailserver remote rules driver
 *
 * @version 1.0
...
class rcube_hmail_remote_rules
{
    ...
    private function data_handler($data)
    {
        ...
        $hmailRemoteUrl = $rcmail->config->get('hms_rules_remote_url', false);
        if (!$hmailRemoteUrl) {
            rcube::write_log('errors', 'Plugin hms_rules (hmail remote driver): $config[\'hms_rules_remote_url\'] is not defined.');
            return HMS_ERROR;
        }
        ...
        elseif($result['error'] != HMS_SUCCESS) {
            rcube::write_log('errors', 'Plugin hms_rules (hmail remote driver): ' . $result['text']);
            return $result['error'];
        }
